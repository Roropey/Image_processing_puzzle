...
prewitt_hor = -1*fspecial('prewitt');
prewitt_ver = prewitt_hor';


BW = edge(image,'prewitt'); %hypot(conv2(image,prewitt_hor),conv2(image,prewitt_ver));
...
if affichage
    subplot(2,2,3); imshow(BW); hold on;
end
...
image_rotate = imrotate(image,rotation);
if affichage
    subplot(2,2,4);
    imshow(image_rotate)
