function outImage = reformImage(Image, R, nbrPieces)
    ...
    h = waitbar(0,'Reconstruction ongoing...');
    steps = 7;
    step = 0;
    ...
    switch nbrPieces
        ...
        case 16
            % Collect vertical area between pieces
            rectVert = [rectVert  outImage(:,sizePieceHoriz+1:2*R+sizePieceHoriz,:)]; 
            % figure; imshow(rectVert(:,1:end,:))
            rectVert = [rectVert  outImage(:,2*R+2*sizePieceHoriz+1:4*R+2*sizePieceHoriz,:)]; 
            % figure; imshow(rectVert(:,end/2+1:end,:))
            rectVert = [rectVert  outImage(:,4*R+3*sizePieceHoriz+1:6*R+3*sizePieceHoriz,:)]; 
            % figure; imshow(rectVert(:,2*end/3+1:end,:))
            step = step + 1;
            waitbar(step / steps)

            % Collect horizontal area between pieces
            rectHoriz = [rectHoriz ; outImage(sizePieceVert+1:2*R+sizePieceVert,:,:)]; 
            % figure; imshow(rectHoriz(1:end,:,:))
            rectHoriz = [rectHoriz  ; outImage(2*R+2*sizePieceVert+1:4*R+2*sizePieceVert,:,:)]; 
            % figure; imshow(rectHoriz(end/2+1:end,:,:))
            rectHoriz = [rectHoriz  ; outImage(4*R+3*sizePieceVert+1:6*R+3*sizePieceVert,:,:)]; 
            % figure; imshow(rectHoriz(2*end/3+1:end,:,:))
            step = step + 1;
            waitbar(step / steps)

            % Remove useless part from original image
            outImage(sizePieceVert+1:2*R+sizePieceVert,:,:) = []; 
            % figure; imshow(outImage)
            rectVert(sizePieceVert+1:2*R+sizePieceVert,:,:) = []; 
            % figure; imshow(rectVert(:,1:end/3,:));
            outImage(2*sizePieceVert+1:2*R+2*sizePieceVert,:,:) = []; 
            % figure; imshow(outImage)
            rectVert(2*sizePieceVert+1:2*R+2*sizePieceVert,:,:) = []; 
            % figure; imshow(rectVert(:,end/3+1:2*end/3,:))
            outImage(3*sizePieceVert+1:2*R+3*sizePieceVert,:,:) = []; 
            % figure; imshow(outImage)
            rectVert(3*sizePieceVert+1:2*R+3*sizePieceVert,:,:) = []; 
            % figure; imshow(rectVert(:,2*end/3+1:end,:))
            step = step + 1;
            waitbar(step / steps)

            outImage(:,sizePieceHoriz+1:2*R+sizePieceHoriz,:) = []; 
            % figure; imshow(outImage)
            rectHoriz(:,sizePieceHoriz+1:2*R+sizePieceHoriz,:) = []; 
            % figure; imshow(rectHoriz(1:end/3,:,:));
            outImage(:,2*sizePieceHoriz+1:2*R+2*sizePieceHoriz,:) = []; 
            % figure; imshow(outImage)
            rectHoriz(:,2*sizePieceHoriz+1:2*R+2*sizePieceHoriz,:) = []; 
            % figure; imshow(rectHoriz(end/3+1:2*end/3+1,:,:))
            outImage(:,3*sizePieceHoriz+1:2*R+3*sizePieceHoriz,:) = []; 
            % figure; imshow(outImage)
            rectHoriz(:,3*sizePieceHoriz+1:2*R+3*sizePieceHoriz,:) = []; 
            % figure; imshow(rectHoriz(2*end/3+1:end,:,:))
            analyze = outImage(:,:,1);
...
%             figure; imshow(outImage(2000:2500,700:1200,:))
%             hold on;
%             rectangle('Position',[200,200,200,200],'EdgeColor','r','LineWidth',2) 
